...
export class CustomHttp extends Http {
  ...
  private handleError(error: any) {
    if (error.status === 401 && !(/api\/authenticate/.test(error.url))) {
      // 401 unauthorized response so log user out of client
      const rediretLocation = '/login?returnUrl=' + encodeURIComponent(window.location.pathname  + window.location.search);
      window.location.href = rediretLocation;
    }
