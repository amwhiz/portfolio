...
var agentParser = require('../lib/agent-parser/agent-parser'),
    agentQueryValidator = require('../lib/agent-query-validator/agent-validator'),
...
exports.getAgents = function(req, res, next) {
    Bluebird
        .resolve(agentQueryValidator.validateGet(req.query))
        .then(function(valid) {
            if (!valid) {
                throw new ParamsValidationError();
            }

            return agentParser.parseCombineGet(req.query);
        })
        .then(function(parsedData) {
            return documentService.getAgents(parsedData);
        })
        .then(function(data) {
            res.sendData(data);
        })
...
exports.getProfiles = function(req, res, next) {
    Bluebird
        ...
        .then(function(valid) {
            ...
            return agentParser.parseGetParams(getLrs(req), req.query);
        })
...
createOrUpdateProfile = function(req, res, next, merge) {
    Bluebird
        ...
        .spread(function(params, files, contentType, lrsId) {
            if (isMultipartType(contentType) && agentQueryValidator.validatePost(params) && _.has(files, 'content') && files.content.length) {
                return Bluebird.all([agentParser.parsePostParams(lrsId, params), documentService.saveFile(files.content[0])]);
            } else if (isMultipartType(contentType) && agentQueryValidator.validatePost(params)) {
                var content = validator.isJSON(params.content) ? [JSON.parse(params.content), contentTypes.JSON] : [params.content, contentTypes.TEXT];

                return Bluebird.all([agentParser.parsePostParams(lrsId, params), content]);
            } else if (isCorrectType(contentType, req.body, contentTypes.FORM_URL, agentQueryValidator.validatePost)
                || isCorrectType(contentType, req.body, contentTypes.JSON, agentQueryValidator.validatePost)) {
                return Bluebird.all([agentParser.parsePostParams(lrsId, req.body), [req.body.content, contentType]]);
            } else if (hasValidParams(req.query, agentQueryValidator.validatePost)) {
                return Bluebird.all([agentParser.parsePostParams(lrsId, req.query), [req.body, contentType]]);
            } else {
                throw new ParamsValidationError();
            }
        })
...
exports.deleteProfiles = function(req, res, next) {
    Bluebird
        .resolve(agentQueryValidator.validatePost(req.query))
        .then(function(valid) {
            if (!valid) {
                throw new ParamsValidationError();
            }

            return agentParser.parsePostParams(getLrs(req), req.query);
        })
        .then(function(params) {
